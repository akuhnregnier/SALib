...
def analyze(problem, y, n_perms, n_var, n_outer, n_inner, n_boot=1):
    ...
    output_sample_2_full = y[n_var:].reshape(
        (n_perms, dim - 1, n_outer, n_inner, n_realization)
    )
    ...
    for i in range(n_boot):
        ...
        # Cost estimation
        c_hat[:, :, i] = np.concatenate(
            (c_mean_var, [var_y.reshape(1, -1)] * n_perms), axis=1
        )
    ...
    if estimation_method == "random":
        ...
        shapley_indices_SE = np.sqrt(
            (shapley_indices_2 - shapley_indices[:, 0] ** 2) / n_perms
        )

        total_indices_2 = total_indices_2 / N_total_2 / output_variance_2**2
        total_indices_SE = np.sqrt(
            (total_indices_2 - total_indices[:, 0] ** 2) / N_total_2
        )

        first_indices_2 = first_indices_2 / N_first_2 / output_variance_2**2
        first_indices_SE = np.sqrt(
            (first_indices_2 - first_indices[:, 0] ** 2) / N_first_2
        )
    else:
        pass

    first_indices = 1.0 - first_indices
