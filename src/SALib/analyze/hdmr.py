...
@mark_dependency
def _compute(X, Y, settings, init_vars, *, verbose=False):
    """Compute HDMR"""
    ...
    for k in tqdm(range(K), desc="HDMR bootstrapping", disable=not verbose):
...
def _print(Si, d):
    ...
    format1 = "%-20s   \t %5.2f (\261%.2f) %5.2f (\261%.2f) %5.2f (\261%.2f) %5.2f (\261%.2f)    %-3.0f"
    format2 = "%-20s   \t %5.2f (\261%.2f) %5.2f (\261%.2f) %5.2f (\261%.2f)                  %-3.0f"
    ...
    format3 = "%-20s   \t %5.2f (\261%.2f) %5.2f (\261%.2f) %5.2f (\261%.2f)"
    print(
        format3
        % (
            "Sum",
...
@cache(
    ...
    ignore=["print_to_console", "verbose"],
)
def hdmr_analyze(
    problem: Dict,
    X: np.ndarray,
    ...
    SA, Em, RT, Y_em, idx = _compute(X, Y, settings, init_vars, verbose=verbose)
