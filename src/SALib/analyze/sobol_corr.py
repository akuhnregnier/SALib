...
def analyze(problem, y, n_sample, n_boot=100, estimator="soboleff2"):
    ...
    """
    assert isinstance(n_boot, int), "The number of bootstrap should be an integer"
    assert isinstance(estimator, str), "The estimator name should be an string"
    assert n_boot > 0, "The number of boostrap should be positive: %d<0" % (n_boot)
    ...
    y = np.array(y)
    for i in range(dim):
        output_sample_i = y[4 * n_sample * i : (i + 1) * 4 * n_sample].reshape(
            4 * n_sample, n_realization
        )
    ...
    for indice_type in ["full", "ind"]:
        ...
        for i in range(dim):
            ...
            first, total = sobol_indices(
                Y1, Y2, Y2t, boot_idx=boot_idx, estimator=estimator
            )
...
def sobol2002_estimator(Y1, Y2, Y2t):
    ...
    first_indice = var_indiv / var
    total_indice = 1.0 - var_total / var
...
def soboleff2_estimator(Y1, Y2, Y2t):
    """ """
    mean2 = m((Y1 + Y2t) / 2.0) ** 2
    var = m((Y1**2 + Y2t**2) / 2.0) - m((Y1 + Y2t) / 2.0) ** 2
...
def sobolmara_estimator(Y1, Y2, Y2t):
    ...
    if True:
        ...
        first_indice = var_indiv / var
        total_indice = var_total / var / 2.0
    else:
        ...
        for i in range(n_realization):
            ...
            total_indice[:, i] = var_total / var / 2.0
...
def cli_parse(parser):
    parser.add_argument(
        "-s", "--samples", type=int, required=True, help="Number of samples"
    )
    parser.add_argument(
        "-r",
        "--resamples",
