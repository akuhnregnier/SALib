...
@mark_dependency
@memoize
def sobol_random_ints(rng, N, num_resamples):
    return rng(N, size=(N, num_resamples))
...
@cache(
    ...
    ignore=["n_processors", "print_to_console", "parallel"],
)
def sobol_analyze(
    problem,
    Y,
    ...
    r = sobol_random_ints(rng, N, num_resamples)
